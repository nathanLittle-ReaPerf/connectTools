...
examples:
  ...
  %(prog)s contacts.json --anonymize
    ...
    p.add_argument("--name",         default=None,  metavar="NAME", help="Scenario name (overrides auto-generated name)")
    ...
    if args.archetypes:
        ...
        for item in archetypes:
            prefix = _safe_filename(args.name) if args.name else date_pfx
            fname = f"{prefix}_{_safe_filename(item['name'])}.json"
            path  = out_dir / fname
    ...
    if args.merge or args.contact_id:
        ...
        if args.name:
            scenario["_name"] = args.name
            fname = f"{_safe_filename(args.name)}.json"
        ...
        for rank, (journey_key, group) in enumerate(top_groups, 1):
            ...
            fname_base = _safe_filename(args.name) if args.name else f"{date_pfx}_journey"
            path = out_dir / f"{fname_base}_{rank:02d}.json"
            path.write_text(json.dumps(scenario, indent=2), encoding="utf-8")
            written.append(str(path))
